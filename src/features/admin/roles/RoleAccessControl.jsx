...
const rolePrivileges = {
  ...
  college: [
    { id: 'manage_college_settings', label: 'Manage College Settings', desc: 'Allows modifying dean information, college branding, and basic details' },
    { id: 'manage_college_departments', label: 'Modify College Departments', desc: 'Add, remove, or rename academic departments inside this college' },
    { id: 'invite_college_staff', label: 'Invite Department Admins & Faculty', desc: 'Send registration emails to prospective collegiate staff' },
    { id: 'approve_college_content', label: 'College Content Approval Queue', desc: 'Accept or reject lectures and videos submitted within the college' },
    { id: 'view_college_analytics', label: 'Access College Analytics', desc: 'View college grade distributions, student retention, and attendance metrics' }
  ],
  department: [
...
export default function RoleAccessControl({ currentRole }) {
  ...
  const [permissions, setPermissions] = useState([
    { email: 'dean@siu.org', role: 'university', label: 'University Admin', scope: 'SIU', features: ['invite_staff', 'approve_content'] },
    { email: 'provost@siu.org', role: 'university', label: 'University Admin', scope: 'SIU', features: ['manage_settings', 'edit_branding'] },
    { email: 'dean.tech@siu.org', role: 'college', label: 'College Admin', scope: 'School of Technology & AI', features: ['manage_college_settings', 'manage_college_departments'] },
    { email: 'hod.cs@siu.org', role: 'department', label: 'Department Admin', scope: 'CS Dept, SIU', features: ['edit_curriculum', 'manage_courses'] },
    { email: 'aisha.patel@siu.org', role: 'faculty', label: 'Faculty', scope: 'CS Dept, SIU', features: ['create_assignments', 'grade_submissions'] },
    { email: 'marcus.williams@siu.org', role: 'faculty', label: 'Faculty', scope: 'CS Dept, SIU', features: ['create_assignments', 'post_content'] },
    { email: 'yuna.park@siu.org', role: 'faculty', label: 'Faculty', scope: 'CS Dept, SIU', features: ['grade_submissions', 'post_content'] },
  ])

  const roleHierarchy = {
    super: [
      { value: 'university', label: 'University Admin' },
      { value: 'college', label: 'College Admin' },
  ...
  const visiblePermissions = permissions.filter(p => {
    ...
    if (currentRole === 'university') return p.role === 'college' || p.role === 'department' || p.role === 'faculty'
    if (currentRole === 'college') return p.role === 'department' || p.role === 'faculty'
