...
import {
  Building2, Building, Users, Activity, HardDrive, DollarSign, Mail,
...
const trafficData = [
  { month: 'Jan', requests: 12.2, target: 10 },
  { month: 'Feb', requests: 13.5, target: 12 },
  { month: 'Mar', requests: 15.0, target: 14 },
  { month: 'Apr', requests: 16.8, target: 15 },
  { month: 'May', requests: 18.4, target: 17 },
  { month: 'Jun', requests: 19.1, target: 18 },
  { month: 'Jul', requests: 21.8, target: 20 },
  { month: 'Aug', requests: 24.2, target: 22 },
  { month: 'Sep', requests: 26.1, target: 24 },
  { month: 'Oct', requests: 28.6, target: 26 },
  { month: 'Nov', requests: 31.0, target: 29 },
  { month: 'Dec', requests: 33.4, target: 31 },
]

const sessionData = [
  { time: '00:00', sessions: 124 },
  { time: '04:00', sessions: 82 },
  { time: '08:00', sessions: 420 },
  { time: '10:00', sessions: 780 },
  { time: '12:00', sessions: 1240 },
  { time: '14:00', sessions: 860 },
  { time: '16:00', sessions: 740 },
  { time: '18:00', sessions: 580 },
  { time: '20:00', sessions: 390 },
  { time: '22:00', sessions: 210 },
]
...
const emailStats = [
  { label: 'Delivered', value: 9482, rate: 97.8, color: 'var(--success)', icon: CheckCircle2 },
  { label: 'Bounced', value: 124, rate: 1.3, color: 'var(--warning)', icon: TrendingDown },
  { label: 'Spam', value: 38, rate: 0.4, color: 'var(--danger)', icon: Shield },
  { label: 'Pending', value: 29, rate: 0.3, color: 'var(--info)', icon: Clock },
]

const universities = [
  { id: 1, name: 'Sricity International University(SIU)', code: 'SIU', region: 'NA', users: 15420, plan: 'Managed', status: 'active', health: 99.8, storage: 142 },
]

const colleges = [
  { id: 1, name: 'School of Technology & AI', university: 'Sricity International University(SIU)', dean: 'Dr. APJ Abdul Kalam', departments: 3, students: 3200, status: 'active' },
  { id: 2, name: 'School of Advanced Manufacturing', university: 'Sricity International University(SIU)', dean: 'Dr. Vikram Sarabhai', departments: 2, students: 1500, status: 'active' },
  { id: 3, name: 'School of Business', university: 'Sricity International University(SIU)', dean: 'Dr. Homi Bhabha', departments: 4, students: 4100, status: 'active' },
  { id: 4, name: 'School of Nove Media', university: 'Sricity International University(SIU)', dean: 'Prof. C.V. Raman', departments: 3, students: 2200, status: 'active' },
]

const auditLogs = [
  { id: 1, action: 'University provisioned', actor: 'system@siu.org', target: 'SIU', time: '2 min ago', severity: 'info' },
  { id: 2, action: 'Feature flag toggled', actor: 'admin@siu.org', target: 'ai_grading → ON (SIU)', time: '14 min ago', severity: 'warning' },
  { id: 3, action: 'Security alert', actor: 'WAF', target: 'Blocked 420 requests', time: '1h ago', severity: 'danger' },
  { id: 4, action: 'New tenant admin', actor: 'system@siu.org', target: 'john@siu.org', time: '1h 30m ago', severity: 'info' },
  { id: 5, action: 'Admin role granted', actor: 'admin@siu.org', target: 'dr.arun@siu.org', time: '2h ago', severity: 'warning' },
  { id: 6, action: 'Domain verified', actor: 'system', target: 'siu.org', time: '3h ago', severity: 'success' },
]
  ...
  return (
    <div style={{ padding: '28px 28px 48px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 14, marginBottom: 20 }}>
        <KpiCard label="Total Universities" value="1" sub="Sricity, AP" icon={Building2} color="#475569" trend="up" trendValue="1 Local Tenant" />
        <KpiCard label="Affiliated Colleges" value="4" sub="Active branch campuses" icon={Building} color="#8B5CF6" trend="up" trendValue="+0 this month" />
        <KpiCard label="Active Users" value="11,500" sub="↑ 284 this week" icon={Users} color="#10B981" trend="up" trendValue="+2.1%" />
        <KpiCard label="Active Sessions" value="1,240" sub="Peak 1,240 at 12 PM" icon={Activity} color="#3B82F6" trend="up" trendValue="+12%" />
        <KpiCard label="Platform Health" value="99.98%" sub="All systems operational" icon={Server} color="#10B981" trend="up" trendValue="SLA met" />
        <KpiCard label="Storage Used" value="1.2 TB" sub="of 5 TB allocated" icon={HardDrive} color="#F59E0B" trend="up" trendValue="24% used" />
      </div>
        ...
        <Card style={{ padding: '20px 20px 12px' }}>
          ...
          <ResponsiveContainer width="100%" height={210}>
            <AreaChart data={trafficData} margin={{ top: 4, right: 4, bottom: 0, left: -10 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: 'var(--text-4)' }} axisLine={false} tickLine={false} tickFormatter={v => `${v}K`} />
      ...
      <Card style={{ marginBottom: 14 }}>
        <div style={{ padding: '18px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 13, fontWeight: 650, color: 'var(--text-1)', marginBottom: 2 }}>Colleges & Affiliated Tenants</div>
            <div style={{ fontSize: 12, color: 'var(--text-3)' }}>Overview of collegiate branches mapped to university parent structures</div>
          </div>
          <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--primary)', background: 'var(--primary-muted)', padding: '4px 10px', borderRadius: 100 }}>
            Active Affiliations
          </span>
        </div>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: 'var(--bg-2)' }}>
                {['College Name', 'Affiliated University', 'Dean', 'Departments', 'Students', 'Status', ''].map(h => (
                  <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, color: 'var(--text-4)', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {colleges.map((c, i) => (
                ...
                >
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      <div style={{ width: 6, height: 6, borderRadius: '50%', background: statusColors[c.status], boxShadow: `0 0 5px ${statusColors[c.status]}` }} />
                      <span style={{ fontSize: 11, fontWeight: 500, color: statusColors[c.status], textTransform: 'capitalize' }}>{c.status}</span>
                    </div>
                  </td>
                  <td style={{ padding: '12px 16px' }}>
                    <button style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-4)', display: 'flex' }}>
