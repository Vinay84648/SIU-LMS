...
const contentQueue = [
  ...
  { id: 2, title: 'Thermodynamics Lab Manual 2026', type: 'document', dept: 'ME', submittedBy: 'Prof. James Liu', status: 'pending', submitted: '4h ago' },
  { id: 3, title: 'Circuit Analysis Problem Set', type: 'document', dept: 'EE', submittedBy: 'Dr. Ana Ruiz', status: 'review', submitted: '6h ago' },
  { id: 4, title: 'Organic Chemistry Slide Deck', type: 'image', dept: 'Chem', submittedBy: 'Prof. Mark Chen', status: 'approved', submitted: '1d ago' },
  { id: 5, title: 'Data Structures — Binary Trees', type: 'video', dept: 'CS', submittedBy: 'Dr. Sarah Kim', status: 'rejected', submitted: '2d ago' },
]
...
const staffInvites = [
  { name: 'Dr. Elena Vasquez', email: 'e.vasquez@mit.edu', role: 'Dept Admin', dept: 'Physics', status: 'pending', sent: '1h ago' },
  { name: 'Prof. Kwame Asante', email: 'k.asante@mit.edu', role: 'Faculty', dept: 'CS', status: 'accepted', sent: '1d ago' },
  { name: 'Dr. Yuki Tanaka', email: 'y.tanaka@mit.edu', role: 'Faculty', dept: 'Math', status: 'pending', sent: '2d ago' },
]
...
const calendarEvents = [
  ...
  { day: 30, label: 'Semester End', color: '#EF4444' },
]
            ...
            {days.map(day => {
              ...
              return (
                <div key={day} style={{
                  ...
                  background: isToday ? 'var(--primary)' : event ? `${event.color}18` : 'transparent',
                  ...
                  title={event?.label}
                >
                  <span style={{ fontSize: 11, fontWeight: isToday ? 700 : 400, color: isToday ? '#fff' : event ? event.color : 'var(--text-2)' }}>{day}</span>
                  {event && !isToday && <div style={{ width: 4, height: 4, borderRadius: '50%', background: event.color, margin: '2px auto 0' }} />}
                </div>
              )
            })}
          </div>
          <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 6 }}>
            {calendarEvents.map(ev => (
              <div key={ev.day} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--text-3)', flex: 1 }}>{ev.label}</span>
                <span style={{ fontSize: 10, color: 'var(--text-4)', fontFamily: "'JetBrains Mono', monospace" }}>Jul {ev.day}</span>
              </div>
            ))}
          </div>
        </Card>
